 ...
 */
public class FrWikiMoneda extends Fragment {
    ...
    private RecyclerView rcWikiMoneda;
    private AdapterRcWikiMoneda adapterRcWikiMoneda;
    ...
    public static FrWikiMoneda newInstance(){
        FrWikiMoneda frWikiMoneda= new FrWikiMoneda();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        rcWikiMoneda =(RecyclerView)rootView.findViewById(R.id.recycleView);
        rcWikiMoneda.setLayoutManager(new LinearLayoutManager(getContext()));
        adapterRcWikiMoneda =new AdapterRcWikiMoneda(getContext(),clickCallBack);
    ...
    public void enviarPeticionJson(){
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET,
                UrlEP.EBFIENDS_W_MONEY
                , null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {

                        listWikiMoneda =parseJsonResponse(response);
                        // MiAplicativo.getWritableDatabase().insertMateriaPensumIndividual(listMateria, ma_modulo, true);//IMPORTANTE
                        adapterRcWikiMoneda.setWikiMonedaList(listWikiMoneda);
    ...
    public ArrayList<WikiMoneda> parseJsonResponse(JSONObject response){
        ArrayList<WikiMoneda> listWikiMoneda = new ArrayList<>();
        ...
        if(response==null || response.length()>0){
            try{
                ...
                JSONArray currentWikiMonedas = response.getJSONArray(Key.JsGetWikiMoneda.WIKI_MONEDA);
                for(int i=0; i<currentWikiMonedas.length();i++){
                    JSONObject currentWMoneda = currentWikiMonedas.getJSONObject(i);

                    if(currentWMoneda.has(Key.JsGetWikiMoneda.TIPO)&&
                            !currentWMoneda.isNull(Key.JsGetWikiMoneda.TIPO)){
                        TIPO=currentWMoneda.getString(Key.JsGetWikiMoneda.TIPO);
                    }
                    if(currentWMoneda.has(Key.JsGetWikiMoneda.DETALLE)&&
                            !currentWMoneda.isNull(Key.JsGetWikiMoneda.DETALLE)){
                        DETALLE=currentWMoneda.getString(Key.JsGetWikiMoneda.DETALLE);
                    }
                    if(currentWMoneda.has(Key.JsGetWikiMoneda.IMG)&&
                            !currentWMoneda.isNull(Key.JsGetWikiMoneda.IMG)){
                        IMG=currentWMoneda.getString(Key.JsGetWikiMoneda.IMG);
                    }

                    WikiMoneda wikiMoneda=new WikiMoneda();
                    wikiMoneda.setTipo(TIPO);
                    wikiMoneda.setDetalle(DETALLE);
                    wikiMoneda.setImg(IMG);
